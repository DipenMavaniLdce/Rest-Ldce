  ...
  var feeRefund ='/student/feeRefund'
  ...
  var xhttp1 = new XMLHttpRequest();
    ...
    xhttp1.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if(x["status1"]=="1"){
                if(x["status2"]=="1"){
                    if(x["status3"]=="1"){ 
                        $("#cardstatus_feeRefund").html("<strong>Approved</strong>");
                    }
                    else if(x["status3"]=="2"){
                        
                        $("#cardstatus_feeRefund").html("<strong>Rejected</strong>");
                        $("#cardstatus_feeRefund").css("display" , "block").append("<strong>"+x["faculty_comment"]+"</strong>");
                    
                    }
                    else{
                        $("#cardstatus_feeRefund").html("<strong>Pending At : Student Section</strong>");  
                        
                    }
                }
                else if(x["status2"]=="2"){
                    $("#cardstatus_feeRefund").html("<strong>Rejected</strong>");
                    $("#cardstatus_feeRefund").css("display" , "block").append("<strong>"+x["comment"]+"</strong>");
                }
                else{
                
                    $("#cardstatus_feeRefund").html("<strong>Pending At : Cleark </strong>");  
                }
                }
                else if(x["status1"]=="2"){
                    $("#cardstatus_feeRefund").html("<strong>Rejected</strong>");
                    ...
                    $("#cardstatus_feeRefund").html("<strong>Pending At : Department</strong>");  
                }
    ...
    xhttp1.open("GET", feeRefund, true);
    xhttp1.send();
