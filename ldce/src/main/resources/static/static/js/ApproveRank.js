    ...
    xhttp.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            function showTableData(x,auth) {
            	...
            	var headRank = ROLE_DEPARTMENT?	`<th>Rank</th>`:``;
            	var headData =`<tr>
                    <th>Sr No.</th>
                    <th>Enroll No.</th>
                    <th>Name</th>
                ...
                for(var i=0;i<x.length;i++) {
                    ...
                    var enroll= x[i].enrollment,
                     ...
                     if(ROLE_DEPARTMENT) 
                    	 var rankField =  `<td><input type="text" id="rank-`+enroll+`" class="form-control rank" ></td>`
                    	 else var rankField =null;
                    var n =
                   ...
                   `+rankField+`
                    <td>
                        <button class="btn btn-info openDocumentModal" data-toggle="modal" data-target="#View_Document"  data-id="`+type+"-"+enroll+`" data-rank=''>View Document</button>
                    </td>
                    <td>
            ...
            $(".btnClick").click(function() {
                ...
                var rank = $("#rank-"+enrollment).val();
                var com = $("#comment"+typ+enrollment).val();
                
                var dataString1={enrollment:enrollment,type:typ,status:this.name,comment:com,rank} ;
