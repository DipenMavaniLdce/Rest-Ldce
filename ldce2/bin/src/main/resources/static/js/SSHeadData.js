...
$("#bth-search").click(function() {
	...
	$('.datarow').remove();
          ...
          console.log(dataString1);
        	     dataString = "http://localhost:8080/admin/ssheaddata?caste="+$("#caste").val()+"&addmission_year="+$("#addmission_year").val()+"&course="+$("#course").val()+"&gender="+$("#gender").val()+"&semester="+$("#semester").val()+"&branch="+$("#branch").val();
...
function showTableData(x) {
    ...
    for(var i=0;i<x.length;i++) {
       ...
       var n =
        `<tr class="datarow">
        <td><input class="checkBoxes" type="checkbox" name="checkBoxArray[]" value="`+enroll+`"></td>
        <td>`+(i+1)+`</td>
        <td>`+enrollment+`</td>
        <td>`+name+`</td>
        <td>`+sem+`</td>
