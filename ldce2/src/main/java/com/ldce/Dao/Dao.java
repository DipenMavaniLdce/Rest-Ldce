...
public class Dao {
	...
	Request request ;
	//saveStudent
	public void save(Student student,Student_info info, Student_guardian guardian, MultipartFile photo,MultipartFile sign) throws IOException {
...
public boolean saveRequest(String enrollment,String type,MultipartFile fee_Receipt) throws IOException {
	...
	if(Document==null) {
		...
		if(Document.isLive()) {
			...
			resetRequest(Document,fee_Receipt);
			Document.setStudent(student);
			requestRepository.save(Document);
			return true;
		}
...
private Request getReq(){return new Request();}
...
public List<Student> pendingRegistration(int branch) {
	...
	public boolean UpdateStatus(userdetails userDetails,String enrollment,String type, String status, String comment) {
		String role=userDetails.getRole();
...
public void resetRequest(Request request,MultipartFile fee_Receipt) throws IOException {
